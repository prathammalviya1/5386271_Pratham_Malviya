...
char** fillBombs(int rows, int cols) {
...
char** detonate(char** grid, int rows, int cols) {
    char** newGrid = fillBombs(rows, cols);
    int dr[] = {-1, 1, 0, 0};
    int dc[] = {0, 0, -1, 1};
    for (int i = 0; i < rows; i++) {
 ...
 *result_count = grid_count;
    ...
    if (n == 1) return copyGrid(grid, grid_count, cols);
    if (n % 2 == 0) return fillBombs(grid_count, cols);
    char** first = detonate(grid, grid_count, cols);
    char** second = detonate(first, grid_count, cols);
